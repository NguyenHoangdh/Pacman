...
PACMAN_COLOR = formatColor(1,0,0)
PACMAN_SCALE = 0.5
#pacman_speed = 0.25
...
# Capsule graphics
CAPSULE_COLOR = formatColor(0,1,0)
CAPSULE_SIZE = 0.5
...
class PacmanGraphics:
    ...
    def drawPacman(self, pacman, index):
        ...
        # ----- Add Eyes -----
        eye_offset_x = self.gridSize * 0.12
        eye_offset_y = self.gridSize * 0.2
        eye_radius = self.gridSize * 0.06   # mắt nhỏ hơn

        left_eye = (screen_point[0] - eye_offset_x, screen_point[1] + eye_offset_y)
        right_eye = (screen_point[0] + eye_offset_x, screen_point[1] + eye_offset_y)

        drawings.append(circle(left_eye, eye_radius, fillColor="white", outlineColor="black"))
        drawings.append(circle(right_eye, eye_radius, fillColor="white", outlineColor="black"))
    ...
    def movePacman(self, position, direction, image):
        ...
        if len(image) > 1:
            eye_offset = self.gridSize * -0.2
            eye_radius = self.gridSize * 0.06

            eye_dx = dx * r * 0.25
            eye_dy = dy * r * 0.25

            left_eye_pos = (
                screenPosition[0] - eye_offset + eye_dx,
                screenPosition[1] + eye_offset + eye_dy
            )
            ...
            moveCircle(image[1], left_eye_pos, eye_radius)
            moveCircle(image[2], right_eye_pos, eye_radius)
